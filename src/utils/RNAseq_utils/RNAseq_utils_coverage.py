...
def calculate_coverage(total_sequences, seq_length):
    """
    ...
    """
    return ((total_sequences * seq_length) * 100) / TOTAL_TRANSCRIPTOME_LENGTH

def process_run_coverage(run_path):
    """
    ...
    for sample, metrics in samples.items():
        ...
        for total_sequences, seq_length in metrics:
            if total_sequences is not None and seq_length is not None:
                coverages.append(calculate_coverage(total_sequences, seq_length))
        if coverages:
            mean_coverage = sum(coverages) / len(coverages)
            coverage_results.append({"Sample": sample, "Mean Coverage": mean_coverage})

    # Convert results into a DataFrame
    coverage_df = pd.DataFrame(coverage_results)
    return coverage_df

def aggregate_runs_coverage(base_dir, process_all=True, specific_run=None):
    """
    ...
    for run_dir in tqdm(run_dirs, desc="Processing runs"):
        run_coverage = process_run_coverage(run_dir)
    ...
    grouped_data.rename(columns={'Mean Coverage': 'Average Mean Coverage'}, inplace=True)
